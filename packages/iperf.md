...
DESCRIPTION
       iperf  2  is  a  tool  for performing network throughput and latency measurements. It can test using either TCP or UDP protocols. It supports both unidirectional and bidirectional
       traffic. Multiple simultaneous traffic streams are also supported. Metrics are displayed to help isolate the causes which impact performance. Setting the enhanced (-e) option pro‐
       vides all available metrics.

       The user must establish both a both a server (to discard traffic) and a client (to generate traffic) for a test to occur. The client and server typically are on different hosts or
...
GENERAL OPTIONS
       ...
       -i, --interval < n[p] | f >
              sample or display interval reports every n seconds (default) or n packets (per optional p suffix.) If f is used then the interval will be each frame or burst. The frame in‐
              terval reporting is experimental.  Also suggest a compile with fast-sampling, i.e. ./configure --enable-fastsampling
...
CLIENT SPECIFIC OPTIONS
       ...
       -t, --time n
           ...
           --txstart-time n.n
              set the txstart-time to n.n using unix or epoch time format (supports microsecond resolution, e.g 1536014418.123456) An example to delay one second using  command  subsitu‐
...
NOTES
       ...
       Reverse,  full-duplex,  dualtest  (-d)  and  tradeoff (-r): The --reverse (-R) and --full-duplex options can be confusing when compared to the older options of --dualtest (-d) and
       ...
       Little's Law in queueing theory is a theorem that determines the average number of items (L) in a stationary queuing system based on the average waiting time (W) of an item within
       ...
       Multicast: Iperf 2 supports multicast with a couple of caveats. First, multicast streams cannot take advantage of the -P option. The server will serialize multicast streams. Also,
